    ...
    return(
        ...
        >
            <span className={styles.value}>{multiple ? value.map((v, index) => (
                <button key={index} onClick={e => {e.stopPropagation(); selectOption(v)}} className={styles['option-badge']}>{v.label} <span className={styles['remove-btn']}>&times;</span></button>
            )) : value?.label}</span>
            <button className={styles["clear-btn"]} onClick={e => { e.stopPropagation(); clearOptions() }}>&times;</button>
